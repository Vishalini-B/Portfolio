...
export default function Experience() {
  return (
    <section id="experience" className="section-padding bg-slate-50 relative overflow-hidden">
      ...
      <div className="absolute top-20 right-[-5%] text-[20vw] font-display font-bold text-slate-200/30 select-none pointer-events-none leading-none">
        WORK
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
          ...
          <motion.p 
            ...
            className="text-slate-500 font-light max-w-xs text-right hidden md:block"
          >
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {experiences.map((exp, i) => (
            <motion.div
              ...
              className="bg-white rounded-[3rem] p-8 md:p-10 border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden"
            >
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-8">
                  <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center ${exp.color.split(' ')[0]} border ${exp.color.split(' ')[2]} shadow-sm group-hover:scale-110 transition-transform`}>
                    <Briefcase size={28} />
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">{exp.role}</p>
                  </div>
                </div>
                ...
                <ul className="space-y-4">
                  {exp.description.map((item, j) => (
                    <li key={j} className="flex gap-4 text-slate-600 leading-relaxed group/item">
                ...
                <div className="mt-10 pt-8 border-t border-slate-50 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-300">Internship Project</span>
