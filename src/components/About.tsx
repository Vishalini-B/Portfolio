...
export default function About() {
  return (
    <section id="about" className="section-padding bg-white dark:bg-slate-950 relative overflow-hidden transition-colors duration-300">
      {/* Grid Background */}
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="aspect-[4/5] max-w-[360px] mx-auto rounded-[4rem] overflow-hidden shadow-2xl relative group">
              ...
              />
              <div className="absolute inset-0 bg-accent/10 mix-blend-multiply opacity-0 group-hover:opacity-100 transition-opacity dark:mix-blend-overlay" />
            </div>
            ...
            <motion.div 
              ...
              className="absolute -bottom-8 -right-4 lg:-right-8 glass p-8 rounded-[3rem] shadow-2xl max-w-[240px] border border-white/50 dark:border-slate-800"
            >
              <div className="flex items-center gap-3 mb-2">
                <Sparkles size={20} className="text-accent" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Education</span>
              </div>
              <p className="text-5xl font-display font-bold text-slate-900 dark:text-white mb-2 tracking-tighter">2027</p>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-tight">B.Tech in Information Technology (Currently Pursuing)</p>
            </motion.div>
          ...
          >
            ...
            <h2 className="text-5xl md:text-6xl font-display font-bold mb-6 leading-[0.9] tracking-tighter text-slate-900 dark:text-white">
              A BLEND OF <br />
              <span className="text-accent italic">DESIGN</span> & <br />
              DEVELOPMENT
            </h2>
            
            <p className="text-xl text-slate-500 dark:text-slate-400 mb-8 leading-relaxed font-light">
              Chennai-based web developer specializing in <span className="text-slate-900 dark:text-white font-medium">React, Node.js, and Tailwind CSS</span>. 
            ...
            <div className="grid sm:grid-cols-2 gap-6 mb-8">
              {[
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl ${item.color} dark:bg-slate-900 dark:text-accent flex items-center justify-center shrink-0 shadow-sm`}>
                    <item.icon size={24} />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white mb-0.5">{item.label}</h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{item.sub}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="p-8 bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
              ...
              <div className="flex flex-wrap gap-2">
                {['Responsive Design', 'Modern UI', 'Clean Code', 'Scalable Apps'].map((tag) => (
                  <span key={tag} className="px-4 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                    {tag}
                  </span>
                ))}
              </div>
